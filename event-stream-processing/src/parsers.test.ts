...
test('apache - safe uri', async () => {
    ...
    const event:KinesisStreamEvent = {
        Records: [
            {
                kinesis:{
                    data: Buffer.from(JSON.stringify({...APACHE_ACCESS_LOG_EVENT_SIGNATURE, message: message1}), 'utf8').toString('base64')
                }
    ...
    await expect(events).toMatchSnapshot('8cb108e0-c707-4faf-83fc-10bc756f6b1b')
});
...
test('apache - safe-ish uri', async () => {
    ...
    const event:KinesisStreamEvent = {
        Records: [
            ...
            {
                kinesis:{
                    data: Buffer.from(JSON.stringify({...APACHE_ACCESS_LOG_EVENT_SIGNATURE, message: message2}), 'utf8').toString('base64')
                }
            } as any as KinesisStreamRecord,
        ]
    }
    ...
    //await expect(events).toMatchSnapshot('4a7f73e8-2b1b-43c9-babd-0a8deb1d09a2')
    for (const event of events) {
        expect(event).toHaveProperty('url.original')
        expect(event).toHaveProperty('url.path')
    }
...
test('apache - unsafe uri', async () => {
    ...
    //await expect(events).toMatchSnapshot('59d34900-3da1-4541-a3fc-2fcd43e2056c')
});
...
test('geoip - client.ip', async () => {
    const event:KinesisStreamEvent = {
        Records: [
            {
                kinesis:{
                    data: Buffer.from(JSON.stringify({...APACHE_ACCESS_LOG_EVENT_SIGNATURE, client:{ip:'2001:569:be94:4700:61b4:917e:808:e3c6'}}), 'utf8').toString('base64')
                }
            } as any as KinesisStreamRecord,
        ]
    }
    ...
    await expect(events).toMatchSnapshot('eb9545ae-e9a8-41bb-8998-81fb0a1931b9')
});
...
test('geoip - source.address', async () => {
    const event:KinesisStreamEvent = {
        Records: [
            {
                kinesis:{
                    data: Buffer.from(JSON.stringify({...APACHE_ACCESS_LOG_EVENT_SIGNATURE, source:{address:'2001:569:be94:4700:61b4:917e:808:e3c6'}}), 'utf8').toString('base64')
                }
            } as any as KinesisStreamRecord,
        ]
    }
    ...
    // await expect([]).toMatchSnapshot('59d34900-3da1-4541-a3fc-2fcd43e2056c')
});
...
test('apache - empty', async () => {
    ...
    await expect(events).toHaveLength(1);
    await expect(events).toMatchSnapshot('eb6509a2-5862-41fc-86a0-8f8e692a9131')
});
...
test('app labels', async () => {
    const event:KinesisStreamEvent = {
        Records: [
            {
                kinesis:{
                    data: Buffer.from(JSON.stringify(lodash.merge({}, APACHE_ACCESS_LOG_EVENT_SIGNATURE, {url:{domain:'a100.gov.bc.ca', path:'/int/irs/hello'}})), 'utf8').toString('base64')
                }
            } as any as KinesisStreamRecord,
        ]
    }
...
test('http event.outcome', async () => {
    const parser = new ParserApplicationClasification()
    const record:any = lodash.merge({}, APACHE_ACCESS_LOG_EVENT_SIGNATURE, {url:{scheme: 'http', domain:'sitesandtrailsbc.ca', path:'/welcome'}})
    parser.apply(record)
    expect(record).toHaveProperty('event.ingested')
    delete record.ingested
    await expect(record).toMatchSnapshot('fe9ed426-57e5-4148-ab8e-0dce6b2c517e')
...
test('app labels - sitesandtrailsbc.ca', async () => {
    const parser = new ParserApplicationClasification()
    const record:any = lodash.merge({}, APACHE_ACCESS_LOG_EVENT_SIGNATURE, {url:{domain:'sitesandtrailsbc.ca', path:'/welcome'}})
    parser.apply(record)
